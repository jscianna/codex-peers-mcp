...
CODEX_PEERS_PORT="$PORT" CODEX_PEERS_DB="$DB_PATH" bun run src/broker.ts >/tmp/codex-peers-broker-test.log 2>&1 &
BROKER_PID=$!
sleep 1
...
peers_json=$(curl -fsS "http://127.0.0.1:${PORT}/peers")
...
msgs_json=$(curl -fsS "http://127.0.0.1:${PORT}/messages?peer_id=peer-b&only_unread=true")
...
after_json=$(curl -fsS "http://127.0.0.1:${PORT}/messages?peer_id=peer-b&only_unread=true")
